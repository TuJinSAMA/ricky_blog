...
在现代 Web 应用中，实时通信已经成为越来越重要的一部分。而 [WebSocket](https://zh.wikipedia.org/wiki/WebSocket) 技术的出现，使得实时通信变得更加高效和便捷。
...
在 `Socket` 类的构造函数 `constructor` 中，我们先取出配置项，把 `query` 内的参数拼接在 URL 上，然后使用 `super` 调用父类的构造函数进行建立 `WebSocket` 连接。
